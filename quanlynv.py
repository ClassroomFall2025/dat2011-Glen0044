...
class Quanlynv:
    ...
    def nhap_dsnv(self):
        ...
        while True:
            ...
            if "hành chính" in bo_phan_lower:
                nv_moi = nhanvien.NhanVien(ma, ho_ten, "Hành chính", luong)
                ...
                nv_moi = nhanvien.TiepThi(ma, ho_ten, "Tiếp thị", luong, doanh_so, ti_le_hoa_hong)
            elif "trưởng phòng" in bo_phan_lower:
                luong_trach_nhiem = self._nhap_float("Nhập Lương trách nhiệm: ")
                nv_moi = nhanvien.TruongPhong(ma, ho_ten, "Trưởng phòng", luong, luong_trach_nhiem)
            else:
        ...
        try:
            with open(FILE_DATA, 'w', encoding='utf-8') as f:
                for nv_obj in self.dsnv:
                    ...
                    if isinstance(nv_obj, nhanvien.TruongPhong):
                        parts.insert(0, 'TP')
                        parts.append(str(nv_obj.luong_trach_nhiem))
                    elif isinstance(nv_obj, nhanvien.TiepThi):
                        parts.insert(0, 'TT')
            ...
            with open(FILE_DATA, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    if loai_nv == 'TP' and len(parts) >= 6:
                        ...
                        nv_obj = nhanvien.TruongPhong(ma, ho_ten, bo_phan, luong, luong_trach_nhiem)
                    elif loai_nv == 'TT' and len(parts) >= 7:
                        doanh_so, ti_le_hoa_hong = parts[5], parts[6]
                        nv_obj = nhanvien.TiepThi(ma, ho_ten, bo_phan, luong, doanh_so, ti_le_hoa_hong)
                    elif loai_nv == 'HC':
                        nv_obj = nhanvien.NhanVien(ma, ho_ten, bo_phan, luong)
    ...
    def cap_nhat_thong_tin_nhan_vien(self):
        ...
        if isinstance(nv_cap_nhat, nhanvien.TruongPhong):
        ...
        elif isinstance(nv_cap_nhat, nhanvien.TiepThi):
